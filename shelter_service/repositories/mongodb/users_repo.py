...
class UserMongoRepository(UserBaseRepository):
    ...
    async def get_by_id(self, id: str) -> User | None:
        ...
        res = await self.collection.find_one(filter={'_id': _id})
        if not res:
            return None
    ...
    async def update(self, user: User, updated_params: dict[str, Any]) -> User:
        ...
        res = await self.collection.replace_one(filter={'_id': _id}, replacement=updated_user_dict)
        return updated_user

    async def delete(self, user: User) -> None:
        _id: ObjectId = ObjectId(user.id)
        await self.collection.delete_one(filter={'_id': _id})
