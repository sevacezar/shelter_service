...
class AdoptionRequestMongoRepository(AdoptionRequestBaseRepository):
    ...
    def _get_request_obj_with_str_ids(self, request_dict: dict) -> AdoptionRequest:
    ...
    async def get_by_id(self, id: str) -> AdoptionRequest | None:
        ...
        if not res:
            return None
        return self._get_request_obj_with_str_ids(request_dict=res)
    ...
    async def update_status(self, request: AdoptionRequest, new_status: str) -> AdoptionRequest:
        ...
        updated_request_dict: dict = updated_request.to_dict()
        del updated_request_dict['id']
        res = await self._collection.replace_one(filter={'_id': _id}, replacement=updated_request_dict)
    ...
    async def list_by_animal(self, animal_id: str) -> list[Any]:
        ...
        async for doc in cursor:
            requests.append(self._get_request_obj_with_str_ids(request_dict=doc))
        return requests
